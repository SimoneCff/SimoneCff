# 👋 Hi, I'm Simone Cioffi

🎓 **MSc student in Machine Learning & Big Data** @ University of Naples "Parthenope"  
🔬 Collaborating on AI research projects with professors at Parthenope  
📑 Co-author of peer-reviewed work on hybrid AI frameworks (vision + FCM inference)  
🏆 Active participant in AI challenges and academic conferences  
🌊 Focus areas: **Computer Vision, Deep Learning, Marine & Medical AI, Open-Source LMMs**

---

## 🚀 Selected Projects
- **[Hyb-DysNet](https://github.com/SimoneCff/Hyb-DysNet)** — Hybrid feature fusion + soft-voting ensemble for dysarthria severity classification in ALS patients - *AIPHEA2026*
- **[ThruCorals](https://github.com/SimoneCff/ThruCorals)** — CNN-based coral classification with SeaThru underwater image enhancement - *BBCC 2023*
- **[MARIS](https://github.com/SimoneCff/MARIS)** — Marine Automated Recognition and Identification System - *BBCC 2024*
- **[Coral Reef Risk Assessment](https://github.com/CI-SSLab/coral-reef-zeroshot-fcm)** — Zero-shot visual feature extraction with Gemma 4-E4B + Fuzzy Cognitive Map inference on Coralscapes dataset - *ICPR 2026- AUSTech workshop*

---

## 🛠️ Core Stack

**ML/DL:** PyTorch · TensorFlow · Keras · scikit-learn · OpenCV  
**Open-Source Models:** Gemma · LLaVA · CLIP · Segment Anything  
**Data:** Python · NumPy · Pandas · Matplotlib  
**Tools:** Docker · Git · Linux · CUDA · AWS  
**Also:** C/C++ · Java · Rust · SQL

---

## 📫 Get in touch

🔗 [LinkedIn](https://www.linkedin.com/in/simone-cioffi-7267211a4) · 🆔 [ORCID](https://orcid.org/0009-0007-4054-7560)
